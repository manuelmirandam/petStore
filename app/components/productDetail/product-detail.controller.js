        ...
        function activate() {
            ProductService.getAll().$loaded()
                .then(function (products) {
                    ...
                    // If there isn't any other product that belongs to the same category, 
                    // we load random products excluding the product itself
                    if (vm.recommendedProducts.length == 0) {
                        vm.recommendedProducts = products.filter(function (x) {
                            return x.$id !== vm.product.$id;
                        });
                    }
                });
        }
